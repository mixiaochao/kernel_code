...
static void do_bh_key(unsigned long data)
{
	...
	if (pdev->cnt%2) {
		...
		input_report_abs(mtdev, ABS_X, mt_axis.x);
		input_report_abs(mtdev, ABS_Y, mt_axis.y);
		input_report_abs(mtdev, ABS_PRESSURE, 1);
	} else {
		input_report_abs(mtdev, ABS_PRESSURE, 0);
...
static int __init keys_init(void)
{
	...
	set_bit(ABS_X, mtdev->absbit);
	set_bit(ABS_Y, mtdev->absbit);
	set_bit(ABS_PRESSURE, mtdev->absbit);

	input_set_abs_params(mtdev, ABS_X, 0, 799, 0, 0);
	input_set_abs_params(mtdev, ABS_Y, 0, 479, 0, 0);
	input_set_abs_params(mtdev, ABS_PRESSURE, 0, 1, 0, 0);
