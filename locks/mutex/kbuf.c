...
static ssize_t kbuf_read (struct file *filp, char __user *buf, size_t cnt, loff_t *fpos)
{
	...
	read_return = copy_to_user(buf, pdev->kbuf, cnt);
...
static ssize_t kbuf_write (struct file *filp, const char __user *buf, size_t cnt, loff_t *fpos)
{
	...
	write_return = copy_from_user(pdev->kbuf + pdev->validlen, buf, cnt); 
	if (write_return != 0) {
		write_return = -EIO;
		goto error0;
	}
...
static int __init kbuf_init(void)
{
	...
	for (i = 0; i < KBUFNUM; i++) {
		...
		//sema_init(&kbufs[i].sema, 1); //init lock
		mutex_init(&kbufs[i].mutex);
	}
