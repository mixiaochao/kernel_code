...
struct proc_dir_entry *ledproc = NULL;
...
static int led_proc_read (char *page, char **start, off_t off,
			  int count, int *eof, void *data)
{
	...
	for (i = 0; i < 4; i++) {
		len += sprintf(page + len, "led %d is %s.\n", i+1, s&(1<<i)? "off":"on");	
	}
...
static int __init demo_init(void)
{
	...
	ledproc = create_proc_read_entry(DEVNAME, 0444, NULL, led_proc_read, NULL);
	if (NULL == ledproc) {
		iounmap(gpm4base);	
	}
